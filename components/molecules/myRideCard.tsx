...
interface MyRideCardProps {
  ...
  date: string;
  onClick: () => void;
  onRateDriver?: () => void; // nova prop
}

export const MyRideCard = ({
  title,
  status,
  origin,
  destination,
    ...
    >
      <div
        className={css({
          ...
          flexShrink: 0,
          shadow: "sm",
          opacity: status === "Recusado" ? 0.75 : 1,
          filter: status === "Recusado" ? "grayscale(1)" : "none",
        })}
      >
        <img
          src="https://lh3.googleusercontent.com/aida-public/AB6AXuCOmSJkFvrw6fpmpqjkLibS417daM4JCOSeWWgi7IoqwjDuYJLSMUyArH7k5sDMhgDwNmOnVXRecNh2Wph-LDgb3gL0Dj-cXbMi_vSEoug3idodGB4rcfaKwt6FcOmllEbFK6Re2rewiB3B_Zq7gNk9ANFVqmP0izG12It8wHXKZpT-JKsiV4ItVMBSi2Ip0hScN_yHxftbTAfzfsZG1HRIi-OqRJGo3qJmTZLyALYtCmaPdVKeK1tfiO9bjAOpcqmd4JwDgMvstOtV"
          alt="Ride"
      ...
      <div className={css({ flex: 1, minW: 0 })}>
        ...
        >
          <h4 className={css({ fontSize: "14px", fontWeight: "semibold", color: "onSurface" })}>
            {title}
          </h4>
        ...
        {status === "Finalizada" && onRateDriver && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onRateDriver();
          ...
          >
            <Icon name="star" size={16} fill /> Avaliar motorista
          </button>
        )}
      </div>
    </div>
